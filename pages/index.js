...
import { getHotDrops, getAllDrops, REVALIDATE_PAGE_CONTENT } from '../lib/graphCMS'
      ...
      <header className='bg-dark-gray relative flex flex-col md:grid md:grid-cols-2 justify-between min-h-[100vh] max-w-[1920px] mx-auto'>
        <div className='flex flex-col md:justify-center md:items-end'>
          ...
          <div className='flex flex-col md:min-h-[400px] lg:min-h-[500px] xl:min-h-[550px] 2xl:min-h-[600px] md:justify-between items-end pl-12 pr-4 md:pr-16 xl:pr-24 2xl:pr-32 xl:pl-12 2xl:pl-0 my-4 md:my-0'>
            <div>
              <h1 className='text-light-blue font-extrabold text-4xl text-right sm:text-5xl md:text-4xl md:pl-6 lg:text-5xl xl:text-6xl xl:pl-0 2xl:text-7xl'>{hotDrops[0].name}</h1>
              <p className='text-white text-right text-2xl md:text-xl mt-1 font-extralight lg:text-2xl 2xl:text-3xl'>{getFormattedDate(hotDrops[0].date)}</p>
            </div>
